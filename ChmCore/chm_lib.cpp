                              ...
                              Int64 len)
{
    ...
    /* NOTE: this might be better done with CreateFileMapping, et cetera... */
    {
        ...
        LARGE_INTEGER origOffset;
        SetFilePointerEx(h->fd, distance, &origOffset, FILE_CURRENT);
        distance.QuadPart = os;
        SetFilePointerEx(h->fd, distance, 0, FILE_BEGIN);
        ...
        SetFilePointerEx(h->fd, origOffset, 0, FILE_BEGIN);
    }
#else
#ifdef CHM_USE_PREAD
#ifdef CHM_USE_IO64
...
#ifdef CHM_MT
#ifdef WIN32
    InitializeCriticalSectionEx(&newHandle->mutex, 0, 0);
    InitializeCriticalSectionEx(&newHandle->lzx_mutex, 0, 0);
    InitializeCriticalSectionEx(&newHandle->cache_mutex, 0, 0);
#else
                      ...
                      void *context)
{
    ...
    strncpy_s(prefixRectified, prefix, CHM_MAX_PATHLEN);
    ...
    while (curPage != -1)
    {
        ...
        while (cur < end)
        {
            ...
            strncpy_s(lastPath, ui.path, CHM_MAX_PATHLEN);
